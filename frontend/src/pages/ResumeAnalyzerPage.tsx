  ...
  return (
    <div className="bg-white/80 backdrop-blur-lg rounded-2xl border border-slate-200/60 shadow-md p-5 space-y-3">
      <div className="flex items-start justify-between">
        ...
        <motion.div
          {...shimmer}
          className="h-5 w-20 bg-slate-200 rounded-full"
        />
      </div>
      <motion.div
        {...shimmer}
      ...
      <motion.div
        {...shimmer}
        className="h-3 w-1/2 bg-slate-100 rounded-full"
      />
      <div className="flex gap-2 pt-1">
        <motion.div
          {...shimmer}
          className="h-8 flex-1 bg-slate-200 rounded-xl"
        />
        <motion.div
          {...shimmer}
          className="h-8 flex-1 bg-slate-200 rounded-xl"
      ...
      >
        ...
        <div className="relative mb-6 flex items-start justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400 ml-10">
              We'll analyse it against the job description instantly
            </p>
          </div>
          <motion.button
        ...
        <div className="relative mb-4">
          <motion.div
            ...
            animate={{
              borderColor: dragOver ? "#6366f1" : file ? "#10b981" : "#e2e8f0",
            }}
            ...
            {file ? (
              <div className="flex items-center justify-center gap-3">
                ...
                <div className="text-left">
                  <div className="text-sm font-bold text-slate-800 truncate max-w-48">
                    {file.name}
                  </div>
        ...
        <div className="relative mb-5 flex flex-col gap-1.5">
          ...
          <motion.input
            ...
            placeholder="SDE, Accountant, etc.."
            className="border border-slate-200 rounded-xl px-3.5 py-2.5 text-sm bg-slate-50/50 text-slate-800 placeholder:text-slate-300 shadow-sm resize-none transition-all duration-200 hover:border-slate-300 hover:bg-white focus:outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-500/25 focus:bg-white focus:shadow-[0_0_0_4px_rgba(99,102,241,0.08)]"
          />
...
export default function ResumeAnalyzerPage() {
  ...
  try {
    setDeleting(id); // you should have: const [deleting, setDeleting] = useState<string | null>(null);
  ...
  if(deleting){
    return (
      <div className="absolute inset-0 z-20 flex items-center justify-center bg-white/70 backdrop-blur-md">
              <ThreeDotsLoader text="Deleting resume.." />
            </div>
    )
                ...
                >
                  <AnimatePresence>
                    {resumes.map((r, idx) => {
                      ...
                      const initials = (r.companyName ?? r.jobTitle ?? "RS")
                        .slice(0, 2)
                        .toUpperCase();
                        ...
                        >
                          ...
                          <div className="flex items-start justify-between gap-2 relative">
                            ...
                            <div className="flex items-center gap-2">
                              {/* Status pill */}
                              <span
                                className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ring-1 shrink-0 ${statusCfg.pill}`}
                              >
                                <motion.span
                                  animate={
                          ...
                          <div className="relative">
                            ...
                            <p className="text-xs text-slate-400 mt-1 line-clamp-2 leading-relaxed">
                              {r.jobDescription?.slice(0, 100)}
                              {r.jobDescription?.length > 100 ? "…" : ""}
                            </p>
                          </div>
